...
internal class OrderControllerTests
{
	...
	{
		_userId = "1";
		_product = new() { Id = 1, OrderId = 1, Name = "Test Product", Price = 10.0m };
		...
		_order = new() { Id = 1, Products = new List<Product> { _product }, UserId = _userId };
		_exception = new(ExceptionMessage);
	}
	...
	public void GetOrder_WhenCalled_ShouldReturnOkResultWithOrder()
	{
		...
		okResult.Value.Should().BeEquivalentTo(_order);
	}
